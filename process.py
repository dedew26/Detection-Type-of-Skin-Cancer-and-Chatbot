...
global responses, lemmatizer, tokenizers, le, model, input_shape
input_shape = 11
...
def load_response():
    global responses
    responses = {}
    with open('dataset/dataset_SC.json') as content:
...
def preparation():
    load_response()
    global lemmatizer, tokenizers, le, model
    tokenizers = pickle.load(open('model/tokenizers.pkl', 'rb'))
...
def vectorization(texts_p):
    vector = tokenizers.texts_to_sequences(texts_p)
